        ...
        for i in range(self.W):
            for j in range(self.H):
                ...
                for r in world.rectangles: 
                    if r.cx - r.w/2 < cx < r.cx + r.w/2 and r.cy - r.h/2 < cy < r.h/2:
                        gt[i, j] = 2; break
...
class RLExplorer:
    ...
    def reset_episode(self, start_pos: Point):
        ...
        return self.world.lidar_pos # Return starting position
    ...
    def _get_action(self, state: Tuple[int, ...]) -> int:
        """ Epsilon-greedy action selection. If epsilon=0, it's pure exploitation. """
        ...
        if random.random() < self.epsilon:
            ...
            # Handle states not yet in Q-table: if all Q-values are 0, choose randomly.
            if np.all(q_values == 0):
                return random.randrange(RL_CONFIG["ACTION_SPACE_SIZE"])
...
def plot_training_results(steps_history: List[int], num_episodes: int, avg_percentage: float = 0.05):
    """ Generates a Matplotlib plot with two subplots: Raw and Adaptive Moving Average. """
    
    # Calculate adaptive window size
    window_size = int(num_episodes * avg_percentage)
    if window_size < 2:
        window_size = min(num_episodes, 2)
    ...
    if len(moving_avg) > 0 and np.min(moving_avg) < 50:
        ax2.axhline(y=np.min(moving_avg), color='r', linestyle='--', alpha=0.7, label=f'Best Average Steps: {np.min(moving_avg):.1f}')
...
def plot_simulation_snapshot(world: World, map_grid: MapGrid, path: List[Point], title: str):
    """ Generates a single plot showing the final map, obstacles, and the path taken. """
    
    fig, ax = plt.subplots(figsize=(8, 8))
    ax.set_title(title, fontsize=14)
    ax.set_xlim(0, WORLD_PX); ax.set_ylim(0, WORLD_PX)
    ax.set_aspect('equal', adjustable='box')
    
    # 1. Draw the Mapped Grid (Visited Cells)
    # Transpose for correct (i, j) indexing to (x, y) plot
    mapped_data = np.transpose(map_grid.visited_grid) 
    
    # Use a color map where 0=Unvisited (White), 1=Visited Free (Light Green), 2=Obstacle (Gray)
    cmap = plt.cm.get_cmap('Greys', 3) 
    cmap.set_under('white') # For 0
    cmap.set_over('gray')   # For 2
    
    # Recolor 1 (Visited Free) to light green
    colors = cmap(np.arange(3))
    colors[1] = [0.6, 0.9, 0.6, 1.0] # Light Green
    cmap = plt.matplotlib.colors.ListedColormap(colors)
    
    ax.imshow(mapped_data, extent=[0, WORLD_PX, 0, WORLD_PX], 
              origin='lower', cmap=cmap, interpolation='none', vmin=0, vmax=2)
    
    # 2. Draw the Obstacles (Ground Truth)
    for r in world.rectangles:
        ax.add_patch(plt.Rectangle((r.cx - r.w/2, r.cy - r.h/2), r.w, r.h, facecolor='darkgray', edgecolor='black'))
    for c in world.circles:
        ax.add_patch(plt.Circle((c.cx, c.cy), c.r, facecolor='darkgray', edgecolor='black'))
        
    # 3. Draw the Robot Path
    if len(path) > 1:
        path_x = [p[0] for p in path]
        path_y = [p[1] for p in path]
        
        # Path trace (dark blue, dashed)
        ax.plot(path_x, path_y, 'b--', linewidth=1.5, alpha=0.7, label='Agent Path')
        
        # Start point (green circle)
        ax.plot(path_x[0], path_y[0], 'go', markersize=8, label='Start Position')
        
        # End point (red star)
        ax.plot(path_x[-1], path_y[-1], 'r*', markersize=12, label='End Position')
...
def replay_best_run(explorer: RLExplorer, start_pos: Point):
    """ Re-runs the simulation using the final learned policy (epsilon=0) and plots the path. """
    
    # Set to pure exploitation
    explorer.epsilon = 0.0
    
    # Reset and get starting position
    path_history: List[Point] = [explorer.reset_episode(start_pos)]
    ...
    while not done and step_count < RL_CONFIG["MAX_STEPS_PER_EPISODE"]:
        ...
        path_history.append(explorer.world.lidar_pos)

    final_coverage = explorer.map_grid.calculate_coverage()
    
    print("\n--- Replay Summary (Best Policy) ---")
    print(f"Steps taken: {step_count} | Final Coverage: {final_coverage*100:.2f}%")
    
    plot_simulation_snapshot(
        explorer.world, 
        explorer.map_grid, 
        path_history,
        title=f"Best Policy Path & Final Map (Steps: {step_count}, Coverage: {final_coverage*100:.1f}%)"
    )
...
if __name__ == "__main__":
    ...
    try:
        plot_training_results(steps_history, num_episodes, avg_percentage=0.05)
    except Exception as e:
        print(f"\n[ERROR] Could not generate Matplotlib training plot: {e}")
        
    # 2. Replay the Best Policy for Visualization
    try:
        replay_best_run(explorer, start_pos)
    except Exception as e:
        print(f"\n[ERROR] Could not generate Replay visualization: {e}")